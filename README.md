...
| ------- | -------------------------------- | --------------------------------- | ---------- |
| Вішліст | `apps/api/src/modules/wishlists` | `apps/web/src/features/wishlists` | `wishlist` |
...
**Вішліст.** Список належить простору, а `owner` — людина, для якої він. Власник **ніколи не бачить
резервацій** (API віддає `reservation: null`), учасники простору бачать, хто зарезервував, гості за
публічним посиланням `/w/<token>` — лише «зайнято». Гість резервує з імʼям і httpOnly-cookie, тож може
скасувати свою резервацію. Ціни — цілі копійки + валюта.
